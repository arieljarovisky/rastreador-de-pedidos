...
export async function generatePostaShippingLabelPdf(
  ...
  if (hasLogo && branding?.logoBuffer) {
    // Caja máx. 34pt de alto (deja lugar al ID de pedido a la derecha); `fit` mantiene el aspect ratio.
    doc.image(branding.logoBuffer, MARGIN, MARGIN, { fit: [CONTENT_WIDTH - 70, 34] });
  } else {
    doc.font(fonts.bold).fontSize(16).fillColor('#1a3a2a').text('Posta', MARGIN, MARGIN, {
      width: CONTENT_WIDTH,
    });
  }
  doc
    ...
    .fontSize(11)
    .fillColor('#1a1a1a')
    .text(order.id, MARGIN, MARGIN, { width: CONTENT_WIDTH, align: 'right' });

  doc
    .moveTo(MARGIN, MARGIN + headerHeight)
    .lineTo(PAGE_WIDTH - MARGIN, MARGIN + headerHeight)
    .strokeColor('#2e6b45')
    .lineWidth(1)
    .stroke();
  ...
  const qrY = MARGIN + headerHeight + 12;
  doc.image(qrPng, qrX, qrY, { width: qrSize, height: qrSize });

  let y = qrY + qrSize + 16;

  doc.font(fonts.bold).fontSize(9).fillColor('#5a6b62').text('DESTINATARIO', MARGIN, y);
  y += 13;
  doc.font(fonts.bold).fontSize(12).fillColor('#1a1a1a').text(order.clientName, MARGIN, y, { width: CONTENT_WIDTH });
  y += doc.heightOfString(order.clientName, { width: CONTENT_WIDTH }) + 2;
  if (order.clientPhone) {
    doc.font(fonts.regular).fontSize(10).fillColor('#1a1a1a').text(order.clientPhone, MARGIN, y);
    y += 16;
  }

  y += 6;
  doc.font(fonts.bold).fontSize(9).fillColor('#5a6b62').text('DIRECCIÓN', MARGIN, y);
  y += 13;
  doc.font(fonts.regular).fontSize(10).fillColor('#1a1a1a').text(order.address, MARGIN, y, { width: CONTENT_WIDTH });
  y += doc.heightOfString(order.address, { width: CONTENT_WIDTH }) + 10;

  if (order.sellerName) {
    doc.font(fonts.bold).fontSize(9).fillColor('#5a6b62').text('VENDEDOR / TIENDA', MARGIN, y);
    y += 13;
    doc.font(fonts.regular).fontSize(10).fillColor('#1a1a1a').text(order.sellerName, MARGIN, y, { width: CONTENT_WIDTH });
  ...
  if (shippingLabel) {
    doc.font(fonts.bold).fontSize(9).fillColor('#5a6b62').text(`Envío: ${shippingLabel}`, MARGIN, y);
    y += 14;
  }
